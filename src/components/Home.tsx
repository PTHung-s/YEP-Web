            ...
            {timelineColumns.map((column, columnIndex) => (
              <div key={columnIndex} className="flex flex-col gap-0 border-2 border-primary bg-surface">
                {column.map((item, itemIndex) => (
                  <div key={`${columnIndex}-${itemIndex}`} className="flex flex-col md:flex-row border-b-2 border-primary hover:bg-surface-container transition-colors cursor-default last:border-b-0">
                    <div className="px-3 py-2 md:w-44 border-b-2 md:border-b-0 md:border-r-2 border-primary bg-primary text-background font-display text-base md:text-lg font-bold flex items-center justify-center tracking-tight whitespace-nowrap">
                      {item.time}
                    </div>
